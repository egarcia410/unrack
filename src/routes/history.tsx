...
import { useHistoryData } from "../features/history/use-history-selectors";
...
const HistoryPage = () => {
  const { liftProgressEntries, recentWorkouts } = useHistoryData();
      ...
      <main>
        ...
        <div className="flex flex-col gap-1.5 mb-6">
          {liftProgressEntries.map((entry) => (
            <LiftProgressCard key={entry.lift.id} entry={entry} />
          ))}
        </div>
